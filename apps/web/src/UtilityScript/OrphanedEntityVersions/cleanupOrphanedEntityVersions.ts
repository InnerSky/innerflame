 ...
 * 
 * This script identifies and removes document_versions records where the corresponding
 * entity (in the documents table) has been deleted, but the versions remain.
 * 
 * It follows these steps:
 * 1. Find all document_versions where the entity_id doesn't exist in the documents table
...
async function cleanupOrphanedEntityVersions() {
  ...
  try {
    // Step 1: Get all entity_ids from document_versions
    ...
    // Step 2: Get all entity_ids from documents
    const { data: allEntities, error: entitiesError } = await supabase
      .from('documents')
      .select('id');
    
    if (entitiesError) {
      throw new Error(`Error fetching documents: ${entitiesError.message}`);
    }
    ...
    // Step 3: Find orphaned versions (those with entity_ids not in documents)
