...
export class DocumentRepository {
  ...
  async getUserDocuments(userId: string): Promise<Document[]> {
    // Fetch all documents
    const { data: documents, error: entitiesError } = await supabase
      .from('documents')
      .select('*')
      .eq('user_id', userId)
    ...
    const { data: versions, error: versionsError } = await supabase
      ...
      .select('*')
      .in('entity_id', documents.map(e => e.id))
      .eq('is_current', true);
  ...
  async getUserProjectsOnly(userId: string): Promise<Document[]> {
    // Fetch all documents of type Project
    const { data: documents, error: entitiesError } = await supabase
      .from('documents')
      .select('*')
      .eq('user_id', userId)
  ...
  async getDocumentsByProject(userId: string, projectId: string): Promise<Document[]> {
    // Fetch all documents with metadata.projectId equal to the provided projectId
    const { data: documents, error: entitiesError } = await supabase
      .from('documents')
      .select('*')
      .eq('user_id', userId)
  ...
  async getDocumentsWithNoProject(userId: string): Promise<Document[]> {
    ...
    const documentsWithNoProject = documents.filter(entity => {
