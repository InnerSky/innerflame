-- First drop the existing function to avoid parameter naming conflicts
DROP FUNCTION IF EXISTS public.delete_user_data(UUID);

-- Create a new function with an unambiguous parameter name
CREATE FUNCTION public.delete_user_data(param_user_id UUID)
RETURNS void AS $$
DECLARE
  entity_ids UUID[];
BEGIN
  -- Step 1: Get all entity IDs belonging to the user
  SELECT array_agg(e.id) INTO entity_ids FROM entities e WHERE e.user_id = param_user_id;
  
  -- Step 2: Delete user's message tags
  DELETE FROM message_tags mt
  WHERE mt.message_id IN (SELECT m.id FROM messages m WHERE m.user_id = param_user_id);
  
  -- Step 3: Delete user's message references
  DELETE FROM message_references mr
  WHERE mr.message_id IN (SELECT m.id FROM messages m WHERE m.user_id = param_user_id);
  
  -- Step 4: Delete all messages from the user
  DELETE FROM messages m WHERE m.user_id = param_user_id;
  
  -- Step 5: Delete all questionnaire responses from the user
  DELETE FROM questionnaire_responses qr WHERE qr.user_id = param_user_id;
  
  -- Step 6: Delete all user events
  DELETE FROM user_events ue WHERE ue.user_id = param_user_id;
  ...
  DELETE FROM users u WHERE u.id = param_user_id;
END;
